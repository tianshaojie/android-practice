package com.android.fine.library.widget.tips;
...
import com.android.fine.library.R;
...
public class TipsView extends LinearLayout {
    ...
    public TipsView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mContext = context;
        LayoutInflater.from(mContext).inflate(R.layout.tips_view,this);
        initAttrs(attrs,defStyleAttr);
        initView();
    }
    private void initAttrs(AttributeSet attrs, int defStyleAttr){
        TypedArray a = mContext.obtainStyledAttributes(attrs, R.styleable.TipsView, defStyleAttr, 0);
        mArrowGravity = a.getInt(R.styleable.TipsView_arrow_gravity, Gravity.BOTTOM | Gravity.RIGHT);
        mText = a.getString(R.styleable.TipsView_android_text);
        mTextSize = a.getDimensionPixelSize(R.styleable.TipsView_android_textSize, mTextSize);
        mTipsColor = a.getColor(R.styleable.TipsView_tips_color, Color.parseColor("#b97aff"));
        mTextGravity = a.getInt(R.styleable.TipsView_android_gravity, Gravity.LEFT);
        mArrowMarginLeft = a.getDimensionPixelSize(R.styleable.TipsView_arrow_margin_left,0);
        mArrowMarginRight = a.getDimensionPixelSize(R.styleable.TipsView_arrow_margin_right,0);
        a.recycle();
    }
    ...
    private void initArrow(){
        ...
        mArrowImage.setImageResource(R.drawable.tips_view_arrow);
        mArrowImage.setScaleType(ImageView.ScaleType.FIT_XY);
        LayoutParams layoutParams = new LayoutParams(dip2px(mContext,10),dip2px(mContext,5));
    ...
    private void setArrow(){
        LayoutParams layoutParams = (LayoutParams) mArrowImage.getLayoutParams();
        if((Gravity.VERTICAL_GRAVITY_MASK & mArrowGravity) == Gravity.BOTTOM){
